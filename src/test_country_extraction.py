...
class TestExtractCountries:
    ...
    def test_successful_extraction(self, mock_get, sample_countries_data):
        ...
        # Assertions
        mock_get.assert_called_once_with("https://restcountries.com/v3.1/all", timeout=10)
...
class TestFetchTranslations:
    ...
    def test_fetch_translations(self, mock_extract_translation, sample_countries_data, sample_translation_data):
    ...
    @patch('country_extraction.extract_translation')
    def test_fetch_translations_with_failures(self, mock_extract_translation, sample_countries_data):
        ...
        successful_count = sum(1 for c in sample_countries_data 
                              if next(iter(c['translations'].values()))['official'] == "République fédérale d'Allemagne")
...
class TestRun:
    ...
    def test_successful_run(self, mock_save_to_s3, mock_fetch_translations, 
                           mock_extract_countries, sample_countries_data, sample_translation_data):
        ...
        mock_save_to_s3.assert_any_call(
            bucket_name, 
            f"countries/{today}/countries.json", 
            sample_countries_data
        )
        mock_save_to_s3.assert_any_call(
            bucket_name, 
            f"countries/{today}/translations.json", 
    ...
    @patch('country_extraction.fetch_translations')
    @patch('country_extraction.save_to_s3')
    def test_run_with_no_countries(self, mock_save_to_s3, mock_fetch_translations, mock_extract_countries):
...
@pytest.mark.parametrize("countries_data,expected_calls", [
    ...
    ([
        {"name": {"common": "Germany"}, "translations": {"fra": {"official": "Test1"}}},
        {"name": {"common": "France"}, "translations": {"fra": {"official": "Test2"}}},
        {"name": {"common": "Spain"}, "translations": {"fra": {"official": "Test3"}}}
    ], 3)
])
def test_fetch_translations_parametrized(countries_data, expected_calls):
    ...
    with patch('country_extraction.extract_translation', return_value={"test": "data"}) as mock_extract:
...
@patch('requests.Session.get')
@patch('boto3.client')
def test_integration(mock_boto_client, mock_session_get, sample_countries_data, sample_translation_data):
